...
背包问题的判定：   
给定一个target，target可以是数字也可以是字符串（可以是1维度，也可是多维度，例如474一和零的各自使用次数上限），    
再给定一个数组nums，nums中装的可能是数字，也可能是字符串，问：能否使用nums中的元素做各种排列组合得到target？      

背包问题技巧：    
1.如果是0-1背包，即数组中的元素不可重复使用，nums放在外循环，target在内循环，且内循环倒序。 （494）     
```
for num in nums:
    for i in range(target, num-1, -1):
````
2.如果是完全背包，即数组中的元素可重复使用，nums放在外循环，target在内循环，且内循环正序。（518）    
```
for num in nums:
    for i in range(nums, target+1):
```
3.如果组合问题中不同顺序的相同元素序列算做不同，需将target放在外循环，将nums放在内循环。（377）          
```
